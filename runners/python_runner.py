"""Wrapper runner pour services Python."""
...
class PythonRunner(RunnerBase):
    def start(self) -> dict:
        ...
        if not entrypoint:
            return {"status": "error", "detail": "entrypoint requis pour runner python"}

        entrypoint_path = Path(self.spec.working_dir) / entrypoint
        if not entrypoint_path.exists():
            return {"status": "error", "detail": f"entrypoint introuvable: {entrypoint_path}"}

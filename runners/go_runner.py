"""Wrapper runner pour services Go."""
...
class GoRunner(RunnerBase):
    def start(self) -> dict:
        working_dir = self.spec.working_dir
        goroot = self.spec.env.get("GOROOT") or os.environ.get("GOROOT")
        command = self.spec.command or ["go", "run", "."]
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                proc = subprocess.Popen(command, cwd=str(working_dir), env=env, start_new_session=True)
